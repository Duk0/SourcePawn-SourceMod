...
new Handle:g_Cvar_Headshot_Attacker = INVALID_HANDLE
...
{
	CreateConVar("sm_dod_dropweapon_version", PLUGIN_VERSION, "Version of sm_dod_dropweapon", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
	...
	g_Cvar_Headshot_Attacker = CreateConVar("sm_dod_dropweapon_headshot_attacker_message", "You made a headshot for")
	g_Cvar_Show = CreateConVar("sm_dod_dropweapon_showmessages", "1", "0 to disable messages, 1 to all, 2 to attacker")
	
	HookEvent("player_hurt", PlayerHurtEvent)
	AutoExecConfig(true, "sm_dod_dropweapon")
}
				...
				if (!(StrEqual(weapon, "bazooka") || StrEqual(weapon, "pschreck") || StrEqual(weapon, "frag_us") || StrEqual(weapon, "frag_ger") || StrEqual(weapon, "riflegren_us") || StrEqual(weapon, "riflegren_ger")))
				{
					...
					if (hitgroup == 1)
					{
						if (GetConVarInt(g_Cvar_Show) == 1)
						{
							...
							if (strlen(message) > 0)
							{
								PrintToChatAll("\x01\x04[SM] %N %s", attacker, message)
							}
						}
						else if (GetConVarInt(g_Cvar_Show) == 2)
						{
							GetConVarString(g_Cvar_Headshot_Attacker, message, sizeof(message))
							if (strlen(message) > 0)
							{
								PrintToChat(attacker, "\x01\x04[SM] %s %N", message, client)
							}
						}
					}
				}
