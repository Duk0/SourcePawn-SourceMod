...
{
	CreateConVar("sm_medic_version", PLUGIN_VERSION, "Version of sm_medic", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
	Cvar_Enabled 			= CreateConVar("sm_medic_enable", "1", " enables/disables the Medic mod")
	Cvar_MinHealth 			= CreateConVar("sm_medic_health", "25", " The minimum health before healing can take place")
	Cvar_MedicDelay 		= CreateConVar("sm_medic_delay", "1.0", " The delay before healing can take place (0.1 to disable)")
	Cvar_Yell		 		= CreateConVar("sm_medic_yell", "3.0", " delay allowed between calling for a medic (3.0 default)")
	Cvar_MedicKits		 	= CreateConVar("sm_medic_kits", "2", " amount of times per life to be able to use !medic (2 default)")
	Cvar_Freeze		 		= CreateConVar("sm_medic_freeze", "1", " 0 = no freezing 1 = freezing (1 default)")
	Cvar_FreezeTime 		= CreateConVar("sm_medic_freeze_time", "3.0", " time to freeze player on healing (3.0 default)")
	Cvar_HealthMinRefund 	= CreateConVar("sm_medic_minhealth", "30", " minimum amount of health to give back to player(30 default)")
	Cvar_HealthMaxRefund 	= CreateConVar("sm_medic_maxhealth", "50", " maximum amount of health to give back to player(50 default)")
	Cvar_HealthRandom		= CreateConVar("sm_medic_randomhealth", "0", " 0 = health refund equals sm_medic_maxhealth cvar 1 = random number between sm_medic_minhealth and sm_medic_maxhealth (1 default)")
	Cvar_HP					= CreateConVar("sm_medic_hp", "100", " set a player's health at respawn")
	Cvar_FTB				= CreateConVar("sm_medic_ftb", "1", " 0 does not fade the screen, 1 fades the screen (default: 1)")
	Cvar_Alpha 				= CreateConVar("sm_medic_alpha", "210", " level of alpha to apply to FTB - 255 is completely opaque 0 is transparent (default: 210)")
	Cvar_Red				= CreateConVar("sm_medic_red", "0", " amount of red color (default: 0)")
	Cvar_Green				= CreateConVar("sm_medic_green", "0", " amount of green color (default: 0)")
	Cvar_Blue				= CreateConVar("sm_medic_blue", "0", " amount of blue color (default: 0)")
	Cvar_Chat				= CreateConVar("sm_medic_chat", "2", " 0=no chat 1=minimal chat 2=full chat")
	...
	AutoExecConfig(true, "sm_dod_medic")
}
...
public Action:Heal(Handle:timer, any:client)
{
	...
	{													
		...
		{	
			...
			EmitSoundToAll("bandage/bandage.mp3", client, SNDCHAN_WEAPON, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL)
