...
public OnPluginStart()
{
	...
	CustBanCommand = CreateConVar("dod_tms_custombancmd", "sm_ban", "<command> = custom ban command to use for banning")
...
{
	decl String:Mapname[128], String:CfgPath[144]
	GetCurrentMap(Mapname, sizeof(Mapname))
	Format(CfgPath, sizeof(CfgPath), "cfg/maps/%s.cfg", Mapname)
	if (FileExists(CfgPath))
		ServerCommand("exec maps/%s.cfg", Mapname)
...
public NativeTMSIsWhiteListed(Handle:plugin, numParams)
{
	...
	new Handle:kv = CreateKeyValues("DoDTMS_WhiteList")
	decl String:steamid[64]
	GetClientAuthId(client, AuthId_Steam3, steamid, sizeof(steamid))
	FileToKeyValues(kv, TMSWhiteList)
	if(!KvJumpToKey(kv, steamid))
	{
		CloseHandle(kv)
		return false
	}
	new immunity = KvGetNum(kv, feature, 0)
	CloseHandle(kv)
	if(immunity == 1)
	{
		return true
...
public NativeTMSIsBlackListed(Handle:plugin, numParams)
{
	...
	decl String:feature[64]
	GetNativeString(2, feature, sizeof(feature))
	new Handle:kv = CreateKeyValues("DoDTMS_BlackList")
	decl String:steamid[64]
	GetClientAuthId(client, AuthId_Steam3, steamid, sizeof(steamid))
	FileToKeyValues(kv, TMSBlackList)
	if(!KvJumpToKey(kv, steamid))
	{
		CloseHandle(kv)
		return false
	}
	new denied = KvGetNum(kv, feature, 0)
	CloseHandle(kv)
	if(denied == 1)
	{
		return true
