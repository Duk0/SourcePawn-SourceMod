...
public OnPluginStart()
{
	...
	CreateConVar("sm_rate_version", PLUGIN_VERSION, "RateChecker Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY)
...
public Action:Cmd_Rate(client, args)
{
	...
	new target = FindTarget(client, name, false, false)
...
public RatesPrint(client,target)
{
	...
	new Handle:menupanel = CreatePanel(GetMenuStyleHandle(MenuStyle_Radio))
	
	new String:targetname[32], String:text[60]
	GetClientName(target, targetname, 31)
	Format(text, 59, "Rates for: %s", targetname)
	
	SetPanelTitle(menupanel, text)
	
	DrawPanelItem(menupanel, " ", ITEMDRAW_SPACER|ITEMDRAW_RAWLINE)
	Format(text, 59, "Rate: %i", irate)
	DrawPanelItem(menupanel, text)
	...
	SendPanelToClient(menupanel, client, RateMenu, 20)
