...
// ------
//
// dod_restock_source <1/0>				=	enable/disable players being able to restock
//
// dod_restock_maxcountperlife <#/0>	=	number of restocks per life a player can use
//											0 = no limit
//
// dod_restock_delaybetweenuse <#/0>	=	number of seconds after restocking can be used again
//											0 = no limit
//
// dod_restock_announce <1/2/0>			=	set announcement
//											1 = only on first spawn
//											2 = every spawn until it is used
//											0 = no announcements at all
//
// dod_restock_ammocheck <1/0>			=	enable/disable disallowing to restock if player has
...
new g_iAmmo
new String:RestockDone[] = "items/ammo_pickup.wav" // weapons/ammopickup.wav, object/object_taken.wav
...
public OnPluginStart()
{
	...
	AutoExecConfig(true, "dod_restock_source")
...
public Action:OnPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(IsClientInGame(client) && IsPlayerAlive(client) && GetClientTeam(client) > 1 && !g_UsedRestock[client] && GetConVarInt(RestockEnabled) == 1 && announce != 0)
	{
		...
		PrintToChat(client, "\x04[DoD Restock] \x01%T", "AnnounceRestock", client, restock)
	}
	return Plugin_Continue
}
...
public Action:cmdRestock(client, args)
{
	if(IsClientInGame(client) && IsPlayerAlive(client))
	{
		...
		{
			PrintToChat(client, "\x04[DoD Restock] \x01%T", "PluginDisabled", client)
		...
		else
		{
			...
			if(WeaponID != -1)
			{
				...
				{
					PrintToChat(client, "\x04[DoD Restock] \x01%T", "NoMeleeRestock", client, Weapon)
				...
				else
				{
					...
					if(AmmoRestock == 0)
					{
						PrintToChat(client, "\x04[DoD Restock] \x01%T", "WeaponDisabled", client, Weapon)
						RestockSound(client, RestockDenied)
						return Plugin_Handled
					}				
					...
					if((GetGameTime() < g_LastRestock[client] + delay) && g_RestockCount[client] != 0 && delay != 0)
					{
						PrintToChat(client, "\x04[DoD Restock] \x01%T", "RecentlyRestocked", client, RoundToCeil(g_LastRestock[client] + delay - GetGameTime()))
						RestockSound(client, RestockDenied)
						return Plugin_Handled
					}
					if(g_RestockCount[client] >= maxrestocks && maxrestocks != 0)
					{
						PrintToChat(client, "\x04[DoD Restock] \x01%T", "RestockLimit", client,  g_RestockCount[client])
						RestockSound(client, RestockDenied)
						return Plugin_Handled
					}
					...
					if(GetConVarInt(RestockAmmoCheck) == 1)
					{
						...
						{
							PrintToChat(client, "\x04[DoD Restock] \x01%T", "RestockEnoughAmmo", client, Weapon)
							RestockSound(client, RestockDenied)
							return Plugin_Handled
						}
					}
					...
					EmitSoundToAll(NeedAmmo[team], client, SNDCHAN_VOICE, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL)
					if(StrContains(Weapon, "riflegren") != -1)
					{
						AmmoRestock--
					}
					SetEntData(client, WeaponAmmo, AmmoRestock, 4, true)
					PrintToChat(client, "\x04[DoD Restock] \x01%T", "RestockSuccess", client, Weapon)
					//RestockSound(client, RestockDone)
					EmitSoundToAll(RestockDone, client, SNDCHAN_WEAPON, SNDLEVEL_NORMAL, SND_NOFLAGS, SNDVOL_NORMAL, SNDPITCH_NORMAL)
	...
	{
		PrintToChat(client, "\x04[DoD Restock] \x01%T", "DeadRestock", client)
		return Plugin_Handled
	}
	return Plugin_Handled
}
